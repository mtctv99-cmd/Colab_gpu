...
def run_tts(model: Any, text: str, reference_wav: str, language: str | None = None, ref_text: str | None = None) -> bytes:
    ...
    try:
        # Sử dụng ref_audio làm tham số chính cho voice cloning theo chuẩn OmniVoice
        audio = model.generate(text=text, ref_audio=reference_wav, **kwargs)
    except TypeError as e:
        print(f"  [warn] model.generate failed with kwargs: {e}. Thử lại bản rút gọn...", flush=True)
        audio = model.generate(text=text, ref_audio=reference_wav)
    ...
    # OOM prevention: clear GPU cache after each inference
    if torch.cuda.is_available():
...
async def heartbeat_loop(ws: Any, interval_seconds: int = 30) -> None:
    while True:
        ...
        while True:
            try:
                ...
                async with websockets.connect(
                    ...
                    await ws.send(json.dumps({"action": "register", "email": email, "gpu": gpu_name, "heartbeat_supported": True}))
